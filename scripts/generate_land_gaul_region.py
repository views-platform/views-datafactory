...
_CODE_FILES = ["gaul0_code", "gaul1_code", "gaul2_code"]
_NAME_FILES = ["gaul0_name", "gaul1_name", "gaul2_name", "iso3_code"]


def _completeness_valid(gaul_dir: Path) -> set[int]:
    """Return gids where all 7 GAUL fields carry a real value."""
    valid: set[int] | None = None
    for name in _CODE_FILES:
        t = pq.read_table(gaul_dir / f"{name}.parquet")
        gids = t.column("gid").to_pylist()
        vals = t.column("value").to_pylist()
        s = {g for g, v in zip(gids, vals, strict=True) if v != -1}
        valid = s if valid is None else valid & s
    for name in _NAME_FILES:
        t = pq.read_table(gaul_dir / f"{name}.parquet")
        gids = t.column("gid").to_pylist()
        vals = t.column("value").to_pylist()
        s = {g for g, v in zip(gids, vals, strict=True) if v}
        valid = valid & s  # type: ignore[operator]
...
def generate(
    ...
    gaul0_valid = {gid for gid, code in zip(gids, codes, strict=True) if code != -1}
    logger.info("GAUL-assigned cells (gaul0 != -1): %d", len(gaul0_valid))

    if gaul_dir is None:
        gaul_dir = gaul_parquet.parent
    complete_valid = _completeness_valid(gaul_dir)
    logger.info("GAUL-complete cells (all 7 fields): %d", len(complete_valid))

    if gaul0_valid != complete_valid:
        only_gaul0 = gaul0_valid - complete_valid
        only_complete = complete_valid - gaul0_valid
        msg = (
            f"Proxy/contract divergence: {len(only_gaul0)} cells have "
            f"gaul0 but incomplete metadata, {len(only_complete)} cells "
            f"are complete but gaul0=-1. GIDs (gaul0-only): "
            f"{sorted(only_gaul0)[:10]}"
        )
        raise ValueError(msg)

    gaul_valid = complete_valid
    ...
    append_ledger_entry(ledger_path, {
        "dataset": DATASET_ID,
        "version": "v1",
        "method": "land_intersection_gaul_complete",
