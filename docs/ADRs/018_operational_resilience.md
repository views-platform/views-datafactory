...
- **Mechanism:** three pings from `refresh_pipeline.sh` — the bare success ping before the duration record, the `/fail` ping in `on_failure()`, and the `/start` ping after the `flock` (C-317, drilled 2026-08-10). All fire-and-forget (`|| true`) — monitoring failure never breaks the pipeline. Cited by symbol: this bullet used to name a line range in that script, which had drifted by ~55 lines with nothing detecting it (C-336).
- **Server config:** `HEARTBEAT_URL` is set in `~/.profile` for `views-deploy` (not `.bashrc` — cron runs non-interactive shells).
- **The URL is a SECRET, not configuration** (C-331, amended 2026-08-10). This ADR previously described it as ordinary config, and that framing is what put it on three `curl` command lines. It is a **capability URL**: holding it is sufficient to send a success ping, mark the pipeline healthy, and silence this entire mechanism — no credential, no account, no further access needed. It therefore (a) goes to `curl` on stdin via `-K -`, never as an argv element, because `/proc/<pid>/cmdline` is world-readable and four accounts have shells on the host; (b) is never echoed to a terminal or a transcript; and (c) implies `~/.profile` should be mode 600. What this does **not** close: the process environment (`/proc/<pid>/environ`, owner+root only) and root itself.
