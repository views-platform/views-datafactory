# Class Intent Contract: UcdpDot9Config

**Status:** Active
**Owner:** Simon Polichinel von der Maase
**Last reviewed:** 2026-04-22
**Related ADRs:** ADR-009, ADR-012, ADR-015, ADR-017, ADR-027

---

## 1. Purpose

> Immutable configuration for fetching UCDP/GED .9 Consolidated Monthly data from the UCDP API.

Governs version discovery for the `.9` stream (format `YY.9.MM`), API transport, rate limiting, and storage. Same structure as `UcdpCandidateConfig` but for the `.9` data stream — an undocumented UCDP data product containing exclusive events not available through annual or candidate endpoints.
...
- This class does **not** know about the `.9` version format (`YY.9.MM`) — that is `_dot9_version_string`
...
- `start_year`: int, >= 1, first year for version discovery (default: `DOT9_FIRST_YEAR` = 2018)
- `start_month`: int, in [1, 12], first month for version discovery (default: `DOT9_FIRST_MONTH` = 1)
...
- Used by `fetch_ucdp_dot9` as the sole configuration input
- Discovery probes construct version strings as `YY.9.MM` via `_dot9_version_string`
...
- Registered in the source registry via `register_source("ucdp_dot9", ...)`
...
UcdpDot9Config(start_month=0)  # ValueError

# WRONG: zero retries
UcdpDot9Config(max_retries=0)  # ValueError
```
...
Tests in `tests/test_ucdp_dot9.py`.

---

## End of Contract

This document defines the **intended meaning** of `UcdpDot9Config`.
