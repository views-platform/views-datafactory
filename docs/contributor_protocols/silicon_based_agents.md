...
- Cross architectural boundaries (ADR-002: the DAG must be respected)
...
- Weaken provenance logic (digest computation, ledger writes, staleness detection)
- Alter grid coordinate logic without explicit carbon-based agent direction
...
The following code areas are especially sensitive and require extra care:

- **Provenance logic** -- ledger writes, SHA-256 digest computation, staleness detection
- **Grid coordinate logic** -- pgid assignment, lat/lon generation, spatial alignment
- **Validation logic** -- schema contracts, `__post_init__` checks, fail-loud enforcement
- **Compilation determinism** -- any code affecting bit-identical output guarantees
- **Temporal backbone** -- month_id conversion, date range validation

Errors in these areas can produce silent data corruption that propagates into humanitarian forecasts.

---

## Enforcement Gates

All silicon-based agent-assisted changes must pass:

- `uv run pytest` -- test suite must pass
- `uv run ruff check` -- linting must pass
- `uv run mypy src/` -- type checking must pass (strict mode)

These gates are non-negotiable. The `uv run` prefix is mandatory for all tool invocations.
