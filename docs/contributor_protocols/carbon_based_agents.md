...
of **carbon-based agents** contributing to views-datafactory.
...
- enforcing architectural boundaries (the DAG of 9 packages),
...
All tests must be run via `uv run pytest`.
...
Using silicon-based agents (Claude Code, etc.) does **not**
...
- apply heightened scrutiny to provenance logic, grid coordinates, and validation code,
...
- intent alignment,
- boundary integrity (ADR-002: does this change respect the DAG?),
- semantic clarity (ADR-003: are all semantics declared?),
- and test adequacy (ADR-005: are red/beige/green covered?).
