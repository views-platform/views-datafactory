...
**Drill (2026-07-31, PR #373 — this correction's own PR):** verified by behaviour, not by reading the settings back. An immediate `gh pr merge 373 --squash` was **refused**: `Pull request #373 is not mergeable: the base branch policy prohibits the merge` (`mergeable_state: blocked`), where the identical command had merged #372 outright hours earlier. `gh pr merge 373 --auto --squash` then *armed* (`auto_merge: squash`) instead of merging, and the PR landed only after `lint`/`typecheck`/`test` reported green. **Not drilled:** whether `gh pr merge --admin` can still force a merge past the gate — GitHub documents `enforce_admins: true` as removing that bypass and the setting reads back enabled, but that path has not been tested destructively.
