...
**Last update:** Go-public/PyPI readiness (2026-07-27) — C-318 registered [DEFER] (basic-auth password in cleartext over HTTP; conscious trade-off recorded alongside the repo-public + PyPI decision; cheap fix = domain + Caddy auto-HTTPS; cross-ref C-97 transport-vs-model split). C-319 registered-and-resolved (Tier 2: test_consumer_provenance's in-process main() held the production pipeline lock for pytest lifetime — 42 suite errors on every full run since v1.8.1; fix: VIEWS_PIPELINE_LOCK_HELD=1 around in-process main()). C-320 registered-and-resolved (Tier 3: deploy-gate falsification tests failed in CI — shallow checkout breaks merge-base, no GH_TOKEN breaks gh — CI red on every branch incl. the v1.8.1 release chain, unnoticed; fix: gates skip-with-reason where unanswerable, still enforce locally; lesson: release ritual must confirm CI green on release commits). Pre-public audit: zero committed secrets/UUIDs/tokens; real SSH username + workstation hostname redacted to placeholders (server_quickref, server_operations, deployment guide, 2026-05-25 post-mortem). sshd verification found PasswordAuthentication YES on the server — go-public blocked until disabled (C-88 addendum pending the fix). Header counts: 317→320 IDs, 287→289 resolved, 27→28 open, Tier 4: 14→15. Prior: Ops sitting #353+#340 (2026-07-22) — C-316 resolved (writer lock, PR #359), C-267 stale-heuristic root cause removed, C-317 start-ping shipped (open pending live grace drill), #340 monitoring review closed (healthchecks confirmed: EU-hosted, portable, start/fail pings close the SIGKILL gap). Header counts: 286→287 resolved, 28→27 open, Tier 3: 7→6. Prior: Epic #342 Story 4 (2026-07-21) — ADR-050 recorded; D-41 registered-and-resolved (vocabulary home: datafactory per ADR-050; views-frames alternative rejected — wrong dependency direction; revisit condition recorded). Consumer contract doc published (docs/guides/consumer_contract.md). Prior: Server incident 2026-07-21 — C-316/C-317 registered OPEN (manual export vs cron collision: standalone scripts bypass the pipeline flock, trigger FIRED, digest gate held; SIGKILL trap-bypass alerting blind spot, near-miss at 95.9% RSS). C-267 addendum: live lock misclassified as stale under 9h swap-bound export. Header counts: 315→317 IDs, 26→28 open, Tier 3: 5→7. Prior: v1.7.1 (2026-07-19) — C-315 registered and resolved (source_features readers scanned top-level provenance while assembly nests under "sources"; pre-coverage warnings never fired on real data; caught by verify_consumer_contract's first server run). Lesson: test fixtures must mirror real artifact shapes. Header counts: 314→315 IDs, 285→286 resolved. Prior: Epic #322 "Trust what we serve" Story 5 sync (2026-07-15) — C-300/C-302 resolved (#325: zarr source metadata, declared-extensive warning), C-314 resolved (#320: verdict REAL, falsification SURVIVED 5/5), C-287 re-tiered 4→3 (WDI proximity), C-131 addendum (code-only resolution exposed; operator config in #324), C-273 trigger honored (#326: snapshot tests written with the change). #95/#97 closed via #326 — NB their issue texts mis-cited C-233/C-234, which are unrelated resolved netrc concerns. Header counts: 314 IDs, 282→285 resolved, 29→26 open (Tier 2: 2→1, Tier 3: 4→5, Tier 4: 17→14). Prior: ACLED 2026-01 anomaly (2026-07-06) — C-314 registered OPEN (January 2026 fatality sum 34,727 vs typical ~9k; investigate before model training, #320). Header counts: 313→314 IDs, 28→29 open, Tier 2: 1→2. Prior: Consumer contract verification (2026-07-05) — C-313 registered and resolved (hardcoded end-year defaults in 9 scripts made dynamic; ACLED 2026 was silently clipped at compile). Header counts: 312→313 IDs, 281→282 resolved. Prior: v1.6.1 deploy incident (2026-07-02) — C-311/C-312 registered and resolved (DGP ordering check downgraded to warn-only after contradicting real UCDP data; ACLED store 2x-duplicate corruption rebuilt + uniqueness guard added). C-257 updated with lesson: DGP checks must be verified against real source data before being made blocking. Header counts: 310→312 IDs, 279→281 resolved. Prior: Deploy v1.6.0 readiness (2026-06-29) — C-309/C-310 registered and resolved (merge topology fixed, deploy guide updated). Header counts: 308→310 IDs, 277→279 resolved. Prior: PR #307 review findings (2026-06-28) — C-306/C-307/C-308 registered and resolved (xfail removed, gh CLI error guard added, plan path globbed). Header counts: 274→277 resolved, 305→308 IDs. Prior: ADR-049 pre-merge fixes (2026-06-28) — C-303, C-304, C-305 resolved (provenance counters added, ADR §2 table corrected, config paths aligned). Header counts: 271→274 resolved, 31→28 open, Tier 4: 20→17. Prior: ADR-049 epic (#300, 2026-06-28) — C-303/C-304/C-305 registered via falsification; ADR-003 epic (#290, 2026-06-28) — C-299 resolved, C-300/C-301/C-302 registered; infrastructure test coverage (#280, 2026-06-26) — C-271/C-280/C-276/C-189 resolved; scaling headroom (#274, 2026-06-26) — C-144/C-145 resolved; temporal alignment (#266, 2026-06-26) — C-286/C-156 resolved; count conservation (#258, 2026-06-24) — C-131/C-291/C-249/C-241 resolved; WDI readiness (2026-06-24) — C-223/C-298 resolved; test hardening (#234, 2026-06-24) — C-290/C-294/C-295/C-296/C-297 resolved. Earlier history in git log
...
**Status:** 320 concern IDs assigned (C-28 merged into C-31, C-107 merged into C-60, C-183 merged into C-44, C-44 merged into C-164, C-03 merged into C-176): 289 resolved, 28 open concerns (0 Tier 1, 1 Tier 2, 6 Tier 3, 15 Tier 4, 6 deferred by design; 2 with fired trigger), 8 open disagreements. 167 resolved concerns as full entries + 19 early-archive reference rows + 105 struck-through in active register (289 unique after dedup — 5 appear in both archive and active) + 32 resolved disagreements in archive. 41 disagreement IDs total: 33 resolved, 8 open.
...
| C-318 | 4 | Data server basic-auth password crosses the wire in cleartext (HTTP, no TLS) | Before adding external consumers or serving anything beyond courtesy-protected research data | Server hardening |
...
Cross-ref: C-97 (basic-auth scalability ceiling — the auth *model*; this entry is the auth *transport*), C-88 (SSH exposure — same server, same hardening cluster).
---
