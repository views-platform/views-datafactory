...
**Status:** 234 concern IDs assigned (C-28 merged into C-31, C-107 merged into C-60, C-183 merged into C-44, C-44 merged into C-164, C-03 merged into C-176): 182 resolved, 49 open concerns (2 Tier 2, 8 Tier 3, 33 Tier 4, 6 deferred by design; 1 with fired trigger), 5 open disagreements. 159 resolved concerns as full entries + 19 early-archive reference rows + 4 demoted in active register + 26 resolved disagreements in archive. 31 disagreement IDs total: 26 resolved, 5 open.
...
### C-234: Local `~/.netrc` has stale duplicate entry for zarr server

Local development machine `~/.netrc` has two entries for `204.168.219.108` — the first has a wrong password (`erwcs122`), the second has the correct one (`erwFFcs122`). Python's `netrc` module uses the first match, so local preflight passes only because the first password happens to work from outside the server. The server-side deployment failed with 401 until the correct password was identified. **Trigger: next time local credential validation behaves differently from server-side.** **Fix: remove the stale first entry from `~/.netrc`, keep only the correct password.**
