...
**Recurrence (2026-07-31, PR #371):** the lesson was violated four days after it was written, and the mechanism is worth recording because it will recur. A background CI watcher was chained to the merge with `;` (`watch_ci ; gh pr merge`). The watcher died on a transient `TLS handshake timeout` from `gh api` — a *network* failure, not a verdict — and `;` ran the merge regardless. A dead watcher and a green watcher are indistinguishable to `;`. Correct forms: `&&` on a watcher whose exit code encodes the verdict, or `gh pr merge --auto` and let GitHub hold the merge until checks pass. (Outcome that time: docs-only 42-line change, and CI on the merge commit 9119501 came back green — `lint`, `typecheck`, `test` all success — so nothing broke. The habit is the defect, not the outcome.)
