...
**Last update:** þing-01 execution DF1/DF3 (2026-07-28) — C-322 registered-and-resolved (Tier 3: #369 redaction audit found the GDL ?token= query param leaking into crash tracebacks via requests exception messages → refresh.log; fix at the shared HTTP layer: same-type redacted re-raise, response/request preserved, from-None context suppression, 5 real-exception-shape tests; UCDP/ACLED/netrc flows verified clean). PLATFORM-001 cross-links added to credential_setup.md + quickstart (#367); harvest-token sentence contributed to PLATFORM-001's co-resident section (views-appwrite@bb08d44). Header counts: 321→322 IDs, 290→291 resolved. Prior: Go-public/PyPI readiness (2026-07-27) — C-318 registered [DEFER] (basic-auth password in cleartext over HTTP; conscious trade-off recorded alongside the repo-public + PyPI decision; cheap fix = domain + Caddy auto-HTTPS; cross-ref C-97 transport-vs-model split). C-319 registered-and-resolved (Tier 2: test_consumer_provenance's in-process main() held the production pipeline lock for pytest lifetime — 42 suite errors on every full run since v1.8.1; fix: VIEWS_PIPELINE_LOCK_HELD=1 around in-process main()). C-320 registered-and-resolved (Tier 3: deploy-gate falsification tests failed in CI — shallow checkout breaks merge-base, no GH_TOKEN breaks gh — CI red on every branch incl. the v1.8.1 release chain, unnoticed; fix: gates skip-with-reason where unanswerable, still enforce locally; lesson: release ritual must confirm CI green on release commits). Pre-public audit: zero committed secrets/UUIDs/tokens; real SSH username + workstation hostname redacted to placeholders (server_quickref, server_operations, deployment guide, 2026-05-25 post-mortem). sshd verification found PasswordAuthentication effectively YES (commented-out default) on the internet-open SSH port — fixed same day via sshd_config.d drop-in, key-only verified live (C-88 addendum); go-public gate passed. C-321 registered-and-resolved (Tier 4: no-netrc remote 401 escaped as raw aiohttp ClientResponseError — not an OSError — breaking the documented PermissionError contract; found by the TestPyPI clean-room rehearsal; fixture-shape lesson, C-315 family). Header counts: 317→320 IDs, 287→289 resolved, 27→28 open, Tier 4: 14→15. Prior: Ops sitting #353+#340 (2026-07-22) — C-316 resolved (writer lock, PR #359), C-267 stale-heuristic root cause removed, C-317 start-ping shipped (open pending live grace drill), #340 monitoring review closed (healthchecks confirmed: EU-hosted, portable, start/fail pings close the SIGKILL gap). Header counts: 286→287 resolved, 28→27 open, Tier 3: 7→6. Prior: Epic #342 Story 4 (2026-07-21) — ADR-050 recorded; D-41 registered-and-resolved (vocabulary home: datafactory per ADR-050; views-frames alternative rejected — wrong dependency direction; revisit condition recorded). Consumer contract doc published (docs/guides/consumer_contract.md). Prior: Server incident 2026-07-21 — C-316/C-317 registered OPEN (manual export vs cron collision: standalone scripts bypass the pipeline flock, trigger FIRED, digest gate held; SIGKILL trap-bypass alerting blind spot, near-miss at 95.9% RSS). C-267 addendum: live lock misclassified as stale under 9h swap-bound export. Header counts: 315→317 IDs, 26→28 open, Tier 3: 5→7. Earlier history in git log
...
**Status:** 322 concern IDs assigned (C-28 merged into C-31, C-107 merged into C-60, C-183 merged into C-44, C-44 merged into C-164, C-03 merged into C-176): 291 resolved, 28 open concerns (0 Tier 1, 1 Tier 2, 6 Tier 3, 15 Tier 4, 6 deferred by design; 2 with fired trigger), 8 open disagreements. 167 resolved concerns as full entries + 19 early-archive reference rows + 107 struck-through in active register (291 unique after dedup — 5 appear in both archive and active) + 32 resolved disagreements in archive. 41 disagreement IDs total: 33 resolved, 8 open.
...
**Source:** PRIO IT security guidance, server setup 2026-03-28; go-public sshd audit 2026-07-27
