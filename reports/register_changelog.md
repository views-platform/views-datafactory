...
the operator's home since the 31 July provisioning. Trigger: *"before the next server snapshot or
any new shell account, confirm the file was distributed and deleted."* Neither of those events
happened; it was closed three weeks later because someone went looking.

**Identified before deletion, not after.** `wc -l` and a name-only `grep` confirmed 5 lines and all
five logins — **no password value was printed** into a terminal, a transcript or a log. ~~C-339~~
destroyed `refresh.log` on this same server three weeks earlier by running a command against a path
nobody had checked; the cost of confirming first is one round trip.

**That check found the entry's own date was wrong.** C-323 said the file was generated by
provisioning on 2026-07-31. Its mtime was **Jul 29 02:19**, and an mtime cannot precede the last
write — so the passwords were generated on the 29th and only the Caddyfile lines landed on the 31st.
Two days, and it matters only because a register with approximate dates cannot be used to
reconstruct a sequence, which is one of the things this one is for.

**What made the decision easy was realising the file had no recovery value.** The operator's
hesitation was reasonable — the recipients would probably lose the passwords. But a lost password is
reissued (`caddy hash-password`, replace the line, reload), and that costs the same whether or not
the file exists. Its only capability was *re-reading a live credential*, which is exactly what the
entry objected to. Keeping it would have bought nothing and preserved the whole risk.
