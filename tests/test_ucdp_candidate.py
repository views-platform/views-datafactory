...
class TestDiscoverVersionsGreen:
    ...
    def test_discovers_multiple_versions(self) -> None:
        ...
        responses = [
            MagicMock(
                json=MagicMock(return_value={"TotalCount": 100}),
                raise_for_status=MagicMock(),
            ),
            MagicMock(
                json=MagicMock(return_value={"TotalCount": 50}),
                raise_for_status=MagicMock(),
            ),
            MagicMock(
                json=MagicMock(return_value={"TotalCount": 0}),
                raise_for_status=MagicMock(),
            ),
        ]
...
class TestFetchUcdpCandidateGreen:
    ...
    def test_full_flow_with_mock(self, tmp_path: Path) -> None:
        ...
        discover_responses = [
            MagicMock(
                json=MagicMock(return_value={"TotalCount": 3}),
                raise_for_status=MagicMock(),
            ),
            MagicMock(
                json=MagicMock(return_value={"TotalCount": 0}),
