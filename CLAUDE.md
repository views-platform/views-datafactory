...
- **Assembly** (`scripts/assemble_grid.py`) combines compiled UCDP + ACLED + GHS-POP + static + admin → `data/assembled/`
...
- Not all paths traverse all layers. Synthetic data reaches consumers directly (skips consolidation, viewpoint, and compilation). GHS-POP skips consolidation (single release, ADR-029).
...
- `datafactory_harvester` — data ingestion with pluggable sources: UCDP (annual, candidate, .9), ACLED, GHS-POP, PRIO-GRID static, GAUL admin boundaries (Layer 1, imports provenance + http)
