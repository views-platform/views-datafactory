...
- **Source nodes** (`datafactory_harvester`) produce raw data independently → `data/raw/`
...
- **Assembly** (`scripts/assemble_grid.py`) combines compiled UCDP + ACLED + GHS-POP + GHS-BUILT-S + V-Dem + SHDI + static + admin → `data/assembled/`
...
- `datafactory_harvester` — data ingestion with pluggable sources: UCDP (annual, candidate, .9), ACLED, GHS-POP, GHS-BUILT-S, V-Dem, SHDI, PRIO-GRID static, GAUL admin boundaries (Layer 1, imports provenance + http)
...
- `reports/rd_roadmap11.md` — R&D roadmap (ACLED end-to-end on server, v1.2 complete, next: V-Dem or WDI)
- `reports/product_development_plan11.md` — Product plan with v1.0/v1.1/v1.2/v2.0 gate criteria
- New data source? Read `docs/guides/data_source_integration_guide.md` before writing code
